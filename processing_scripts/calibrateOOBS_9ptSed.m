...
gen_path = "/Users/Ted/GDrive/OpenOBS/SedimentCalibrations/Sag2021/";

concentration = [0, .01, .05, 0.1, 0.2, 0.5, 1, 2, 4, 6];
...
lm = fitlm(measured(:,1),concentration);
TSS = predict(lm,measured(:,1));
        
eb = errorbar(concentration,measured(:,1),measured(:,2));
eb.LineWidth = 1;
...
save(fullfile(save_path,file(1:8)),"measured","concentration","TSS","lm","data")

%% look at a bunch of cal data
date_string = "14092021";
sn_ignore = [10,16];

filteredConcentrations = [5.0,8.7,32.4,94.3,199.0,364.0,484.8,1708.6,4580.9,4111.4];
...
for i = 1:numel(cal_path)
    ...
    concentration = filteredConcentrations;
    concentration(end-1:end) = [];
    measured(end-1:end,:) = [];
    lm = fitlm(measured(:,1),concentration);
    TSS = predict(lm,measured(:,1));
    
    cal_data(i).sn = str2double(cal_path(i).folder(end-2:end));
    cal_data(i).concentration = concentration;
    ...
    subplot(1,2,1)
    hold on
    plot(concentration,measured(:,1),'o-','Linewidth',1.5)
    
    subplot(1,2,2)
    hold on
    plot(concentration+1,TSS'-concentration,'o-','Linewidth',1.5)
