...
void interruptRoutine() {
  ...
  detachInterrupt(digitalPinToInterrupt(pInterrupt));
}
...
void enterSleep(DateTime& dt) { //argument is Wake Time as a DateTime object
  ...
  attachInterrupt(digitalPinToInterrupt(pInterrupt), interruptRoutine, FALLING);
  delay(100); //wait for a moment for everything to complete
  Serial.flush(); //if anything is in TX buffer the light stays on.
  LowPower.powerDown(SLEEP_FOREVER, ADC_OFF, BOD_ON); //power down everything until the alarm fires
}
